...
export default class CourseGroup {
    ...
    constructor(data) {
        ...
        this.course_ids = data.acf.courses
            ? data.acf.courses.map((course) => course.ID)
            : null;
    }
    ...
    appendToHTMLElementTable(parent, idx) {
        parent.insertAdjacentHTML("beforeend", this.getTemplateContainer(idx));
    ...
    appendToHTMLElementCard(parent) {
        ...
        card.querySelector("a").addEventListener(
            "click",
    ...
    showPickOneCourseOptionsInline(parent) {
        parent.insertAdjacentHTML("beforeend", "Pick One:<div></div>");
        const container = parent.lastElementChild;
        this.courses.forEach(
            ((course, idx) => {
                if (idx > 0) {
                    container.insertAdjacentHTML("beforeend", ` &bull; `);
                }
    ...
    appendTable(parent) {
        parent.insertAdjacentHTML("beforeend", CourseGroup.getTemplateTable());
    }
